...
`dotnet remove` says "Found more than one project" — likely because the working directory already contains the `.csproj` and the full path is also specified.
*(TestHelper.cs lines 171–173)*

### 3. Broken: Error text mismatch

`"Invalid NuGet version string"` doesn't match the actual SDK error `"'4.3.' is not a valid version string"`.
*(FirstUseTests.cs line 61)*

### 4. Swallowed errors

`UninstallPackage`, `Build`, and `SetInitialState` results are silently ignored in many places — tests may pass while setup actually failed.
...
Inline into test methods. Remove dead `Cleanup()`.
*(All test classes; TestHelper.cs line 313)*

### 9. Naming

`_h` → `_helper` or `_testHelper`. `CommandResult` → `CommandLineResult`.
...
Timeout from config. Static path init. Embedded resources for NCrunch compatibility.
*(TestHelper.cs lines 34–37, 98)*

### 12. Delete logging accuracy

Log whether file actually existed before deletion.
...
Drop `TestContext` dependency, simplify to `Console`+`Debug`.
*(TestHelper.cs lines 21, 58)*

### 15. Minor

`e.Data ?? ""` null simplification. Assert-before-remove in upgrade test. Use `AssertCore` from `JJ.Framework.Testing.Core`.
